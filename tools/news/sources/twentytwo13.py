...
class TwentyTwo13Scraper(BaseNewsScraper):
    BASE = "https://twentytwo13.my"
    ...
    def _parse_content_blocks(self, element: Tag) -> list[ContentBlock]:
        ...
        blocks = []
        for el in element.find_all(["p", "h1", "h2", "h3", "h4", "ul", "ol", "blockquote"], recursive=True):
            tag = el.name.lower()
    ...
    def get_article(self, url: str) -> Optional[Article]:
        ...
        byline = soup.find(class_="post-author")
        if byline:
            author = byline.get_text(strip=True)

        body = soup.find(class_="single-post-content")
